...
async function generateJWT(payload: { id: string; email: string; role: 'org' | 'corp' }): Promise<string> {
  const jwtToken = jwt.sign(payload, jwtSecret, { expiresIn: '1h' })
  return jwtToken
}

async function verifyJWT(token: string): Promise<{ id: string; email: string; role: 'org' | 'corp' }> {
  try {
